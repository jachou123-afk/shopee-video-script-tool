...
The NAS ERP sync also posts a sanitized rolling order index to the authenticated `/erp/orders/push` route. An allowlisted private LINE user can send a printed shipping-slip number such as `0350000-10747554`, or the explicit `訂單 {編號}` command, to read status, platform order number, items, quantities, prices, total, and shipping details. Group and room sources never receive order data. Recipient/buyer names, phone numbers, and addresses are excluded before upload and are discarded again by the Worker normalizer. The allowlist is configured only through the `LINE_ORDER_ALLOWED_USER_IDS` Worker secret. Order details fail closed if the NAS snapshot is older than 30 minutes.
