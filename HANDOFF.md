...
- 在 `A817 → 查訂單 → 新訂單／可出貨／出貨中` 的結果清單出現後，已授權的一對一 LINE 使用者可於 10 秒內直接輸入畫面上的編號，Worker 會優先精確查詢該張完整出貨單，不會把數字交給既有廣告影片排程。第 2 頁沿用畫面上的 `11～20`，不是重新從 1 開始。
- 選單只保存 ERP 快照版本、貨號、狀態、頁碼、畫面編號範圍與完整訂單查詢鍵；LINE user ID 仍只以 SHA-256 雜湊作儲存鍵，不保存商品、買家或收件資料。成功選取後立即刪除；超過 10 秒、ERP 快照換版或換頁／切換狀態都不沿用舊對應。
- 驗證：`node --check ai-worker/src/index.js`、84/84 項 Worker 測試與 `git diff --check` 通過；涵蓋 10 秒有效期、逾時拒絕、第 2 頁 `11～12` 範圍、一次性選取、數字路由優先、完整訂單明細與 PII 不回覆。功能 commit `6d4d8b2170ec5ede4643a580e388a04c02155769` 已推送到正式 `agent/line-schedule-handoff` 分支；Cloudflare Worker version `2e63d014-e275-4b48-8ead-7c264d89a82b` 已於 2026-09-01 17:52（Asia/Taipei）正式部署並承接 100% 流量。部署後公開根網址為預期 HTTP 405，未授權 `/erp/orders/push` POST 為 HTTP 401。待驗收：在 `@059hdfyo` 私訊依序輸入 `A817`、選 `查訂單` 與一個狀態，再於 10 秒內輸入畫面編號，應直接顯示該張完整訂單。
...
- Latest feature source: `6d4d8b2170ec5ede4643a580e388a04c02155769` (`fix(line): extend SKU order selection to ten seconds`).
- Latest production deployment: `2e63d014-e275-4b48-8ead-7c264d89a82b` on 2026-09-01, containing the ten-second SKU order-number shortcut plus all earlier formal-branch changes.
