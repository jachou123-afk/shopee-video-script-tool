...
The private-chat command `改儲位 {SKU} {新儲位}` currently runs in read-only dry-run mode for `@059hdfyo` only. It verifies the active LINE bot Basic ID, reads a recent `InventoryId=1` snapshot, and previews a single-variant location change. It deliberately performs no ERP write and refuses groups, stale snapshots, non-main warehouses, missing products, and ambiguous multi-variant products.
