...
#### 狀態清單 5 秒數字直達（已正式部署，待 LINE 畫面驗收）

- 在 `A817 → 查訂單 → 新訂單／可出貨／出貨中` 的結果清單出現後，已授權的一對一 LINE 使用者可於 5 秒內直接輸入畫面上的編號，Worker 會優先精確查詢該張完整出貨單，不會把數字交給既有廣告影片排程。第 2 頁沿用畫面上的 `11～20`，不是重新從 1 開始。
- 選單只保存 ERP 快照版本、貨號、狀態、頁碼、畫面編號範圍與完整訂單查詢鍵；LINE user ID 仍只以 SHA-256 雜湊作儲存鍵，不保存商品、買家或收件資料。成功選取後立即刪除；超過 5 秒、ERP 快照換版或換頁／切換狀態都不沿用舊對應。
- 驗證：`node --check ai-worker/src/index.js`、84/84 項 Worker 測試與 `git diff --check` 通過；涵蓋 5 秒有效期、逾時拒絕、第 2 頁 `11～12` 範圍、一次性選取、數字路由優先、完整訂單明細與 PII 不回覆。功能 commit `43329cef25db90c2b18147a9427280aa36920b79` 已推送到正式 `agent/line-schedule-handoff` 分支；Cloudflare Worker version `f49232c3-f616-43f1-b03b-d8c28e9c52f5` 已於 2026-09-01 17:41（Asia/Taipei）正式部署並承接 100% 流量。部署後公開根網址為預期 HTTP 405，未授權 `/erp/orders/push` POST 為 HTTP 401。待驗收：在 `@059hdfyo` 私訊依序輸入 `A817`、選 `查訂單` 與一個狀態，再於 5 秒內輸入畫面編號，應直接顯示該張完整訂單。
...
- 使用者實測 `10745966` 正確列出 16 張後，直接輸入 `11` 卻被既有「蝦皮廣告影片排程」數字路由接走。原因是出貨單清單原本只顯示編號、沒有保存本次待選脈絡。
- 正式版現會在已授權的一對一私訊中，把多張出貨單清單保存為 5 分鐘短效待選狀態；輸入 `11` 會優先精準查詢第 11 張 `0350010-10745966`。選取成功即刪除；輸入超出範圍會提示 `1～16` 且保留本次選單。
- 待選狀態只保存快照版本、交易序號與完整出貨單號，不保存商品或買家資料；LINE user ID 僅以 SHA-256 雜湊作儲存鍵。不同使用者、過期狀態、ERP 快照換版、待更新／截斷清單都不能沿用；選取時仍重新走 exact ERP 訂單查詢，不回放舊明細。
- 沒有有效出貨單待選狀態時，裸數字仍交由原廣告影片排程處理；手動輸入另一個訂單號，或切換到廣告排程新增／列表／完成／取消流程時，都會先清除舊訂單選單。過期或 ERP 快照換版則明確要求重新輸入交易尾號，不會落入廣告回覆。
- 新訂單選單採 fail closed：儲存前先刪除上一筆；若新選單驗證、儲存或清除失敗，就不顯示可能造成誤選的新編號清單，避免數字指向上一筆交易。
- 驗證：`node --check ai-worker/src/index.js`、81/81 項測試與 `git diff --check` 通過；測試包含實際 16 張清單輸入 `11`、數字選取優先序、廣告流程切換、使用者隔離、五分鐘失效、快照換版、儲存失敗清舊、超出範圍、一次性選取、手動查詢清除及原始 user ID 不落地。功能 commit `8e8e3e2`，跨流程安全修正 commit `6b9bc0a`；複核未發現剩餘 P0／P1。
- Cloudflare Worker version `93768856-f1e9-4f2b-8b89-b5e7660d8de3` 已於 2026-08-29 10:49（Asia/Taipei）正式部署；部署後公開根網址為預期 HTTP 405，未授權 `/erp/orders/push` POST 為 HTTP 401。本次只改 Worker，NAS 程式與同步格式不需變更。
- 待驗收：在 `@059hdfyo` 私訊重新輸入 `10745966`，收到 16 張清單後於 5 分鐘內輸入 `11`；預期直接顯示 `0350010-10745966` 的出貨單內容，不再出現蝦皮廣告回覆。

- 使用者已確認查詢規則：完整 `前綴-尾號` 只回該張出貨單；只輸入尾號時，唯一命中直接顯示，多張命中則以自然數字順序列出完整號碼並分頁（每頁 20 張），絕不跨張合併；`全部 {尾號}` 才顯示整筆交易彙總。完整號碼查無資料時不可退回尾號搜尋。
- 回覆改為先列完整出貨單號、平台、平台單號、狀態與出貨方式，再依商品分組；每個規格固定保留 `貨號｜款式｜×數量`，下一行顯示 `儲位｜單價｜小計`。本張商品小計與整筆交易總計分開呈現；缺少 ERP 欄位一律顯示「未提供」，不猜值。
- Worker 會以 30 分鐘內主倉快照的 exact 唯一子貨號補儲位，不覆蓋訂單原值；長訊息保留 ERP 更新時間。舊快照若沒有逐張明細，只會提示等待新版同步，不會偽造或拆分結果。
- 安全界線：NAS／Worker 皆重套 PII 白名單；空、無效、重複交易或明細小計矛盾時停止發布並保留上一版。alias 超量、單 bucket／單訂單超過儲存上限或逐張明細遭截斷時也整批拒絕，避免部分索引被設為 active。
- 驗證：Worker `node --check`、79/79 項測試與 `git diff --check` 通過；來源功能 commit `e3dc5df`。Cloudflare Worker version `cabced97-b532-412b-be31-ce2f5451bdb4` 已正式部署；公開根網址為預期 HTTP 405，未授權 `/erp/orders/push` POST 為 HTTP 401。
- 2026-08-29 10:20:30（Asia/Taipei）Cloudflare 即時記錄確認正式 NAS 對 `/erp/orders/push` 與內部 Durable Object `/erp-orders/sync` 的 POST 均為 HTTP 200，承接版本即 `cabced97-b532-412b-be31-ce2f5451bdb4`，請求長度 6,792,641 Bytes。
- 最新正式快照含 2,595 筆交易、5,061 張完整出貨單。核對：`10747956` 唯一命中 `0354000-10747956`；`10745966` 有 16 張，依序為 `0350000-10745966` 至 `0350015-10745966`；其中 `0350000-10745966` 本張 1 件／NT$25，整筆交易總計 NT$400；`全部 10745966` 彙總為 16 件／NT$400。
- 待新版 LINE 畫面驗收：請依序私訊 `10747956`、`10745966`、`0350000-10745966`、`全部 10745966`。本節的新精準分流尚未以真實 LINE 畫面驗收；下方 02:08 驗收紀錄只代表前一版子貨號／儲位格式。
...
- Latest feature source: `43329cef25db90c2b18147a9427280aa36920b79` (`feat(line): open SKU orders by recent number`).
- Latest production deployment: `f49232c3-f616-43f1-b03b-d8c28e9c52f5` on 2026-09-01, containing the five-second SKU order-number shortcut plus all earlier formal-branch changes.
